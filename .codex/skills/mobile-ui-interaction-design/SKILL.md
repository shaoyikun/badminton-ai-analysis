...
- 顶部标题栏和底部导航是否真正固定，而不是跟着整个页面一起滚走
- 主内容是否是独立滚动容器，还是整个文档被不断撑长
- 页面是否仍然像 PC 缩窄后的文档流，而不是正式移动端任务页
- 步骤表达是否只是文字说明、按钮感卡片或伪进度条
- 主 CTA 是否被埋在超长页面底部
- 候选片段、微调参数、详细指标是否像工具面板一样铺满页面
...
## 常见错误 / Avoid These Mistakes

- 不要默认接受 `position: sticky` + 长文档流的假固定导航；先验证滚动到底发生在 `window` 还是内容容器。
- 不要用 PC 文档流思维做移动端主页面，把所有 section 顺着文档无限加长。
- 不要把多阶段流程压成一个页面，再靠说明文案硬解释步骤感。
- 不要让高级调整和调试型指标与主任务同层常驻。
- 不要只看代码结构判断 UI 是否“已经产品化”；必须配合截图或可视快照复核。
