...
import { assertShellLayout } from './support/helpers'
...
test('首页主漏斗可达性', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { name: '先上传一段完整挥拍视频，再得到可复测的动作结论' }),
  ).toBeVisible()
...
test('上传页默认禁用态', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '上传前快速检查' })).toBeVisible()
...
test('上传页无效文件阻塞提交', async ({ page }) => {
  ...
  await expect(page.getByText('invalid-image.jpg').first()).toBeVisible()
...
test('上传页合规文件解锁提交', async ({ page }) => {
  ...
  await expect(page.getByText('valid-clear.mp4').first()).toBeVisible()
  await expect(page.getByText('8 秒')).toBeVisible()
...
test('粗扫后可选片段并进入处理中页', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/analyses\/[^/]+\/segments$/)
  ...
  await expect(page.getByRole('heading', { name: '系统正在处理你确认过的挥拍片段' })).toBeVisible()
  await expect(page.getByText('当前文件')).toBeVisible()
  await expect(page.getByText('valid-clear.mp4')).toBeVisible()
  await expect(page.getByText('分析片段', { exact: true })).toBeVisible()
...
test('粗扫后可轻量微调当前候选片段时间窗', async ({ page }) => {
  ...
  await page.getByText('高级微调（可选）').click()
...
test('首页切换到杀球后可进入对应上传与处理中链路', async ({ page }) => {
  ...
  await page.getByRole('link', { name: '我已了解，去上传' }).click()
  await expect(page).toHaveURL(UPLOAD_PATH)
  await expect(page.getByText('优先保证身体加载、挥拍臂加载、击球候选到随挥这一整段都拍完整。')).toBeVisible()
