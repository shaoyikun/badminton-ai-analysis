...
function getFeatureSummary(summary: PoseAnalysisResult['summary'], key: string) {
  return getSharedFeatureSummary(summary, key);
}
...
function buildDimensionEvidenceMap(dimensionEvidence: DimensionEvidenceEntry[]) {
  return buildSharedDimensionEvidenceMap(dimensionEvidence);
}
...
function buildPhaseAssessment(
  ...
  return buildSharedPhaseAssessment(phaseKey, profile.phaseLabels[phaseKey], status, summaryText, summary, evidenceRefs);
}
...
function buildEvidenceSentence(evidenceRefs: EvidenceRef[]) {
  return buildSharedEvidenceSentence(evidenceRefs, (score) => `${score ?? '—'} 分`);
}
...
function buildSuggestionDraft(
  ...
  const suggestionType = ruleKey === 'capture_fix'
    ...
    : ruleKey === 'technique_focus'
      ? 'technique_focus'
      : 'retest_check';
  return buildSharedSuggestionDraft(ruleKey, suggestionType, suggestion) as SuggestionDraft;
}
